...
export default function EditEventPage() {
  ...
  const loadEvent = async () => {
    setIsFetching(true);
    const result = await eventsService.getById(params.id);
    ...
    if (result.success) {
      ...
      alert(result.error || "No se pudo cargar el evento.");
      router.push("/eventos");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await eventsService.update(params.id, formData);

      if (result.success) {
        alert(result.message || "Evento actualizado correctamente.");
        router.push("/eventos");
      } else {
        setErrors({ submit: result.error || "No se pudo actualizar el evento." });
      }
    } catch (error) {
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 text-[#1A125C]">
            ...
            <Textarea
              label="Descripción"
              name="descripcion"
              placeholder="Describe en qué consiste el evento, a quién va dirigido, requisitos, etc..."
